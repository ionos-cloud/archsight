...
class Archsight::Import::Handlers::GoGrapher < Archsight::Import::Handler
  ...
  def collect_packages(repo_root, modules, prefix, workspace_mode: false)
    ...
    modules.each do |rel_dir, _mod_name|
  ...
  def l1_topo_depths(pkgs, mod_name)
    ...
    l1_imports = all_l1.each_with_object({}) { |l1, h| h[l1] = Set.new }
  ...
  def module_topo_depths(pkgs, modules)
    ...
    all_dirs = modules.map { |rel_dir, _| rel_dir }.to_set
    mod_imports = all_dirs.each_with_object({}) { |d, h| h[d] = Set.new }
  ...
  def emit_module_cluster(g, mod_name, mod_colors, pkgs_in_mod, prefix)
    cluster_id = mod_name.delete_prefix(prefix).gsub(/[^a-zA-Z0-9]/, "_")
    fill = mod_colors[:fill]

    g.subgraph("cluster_#{cluster_id}",
      ...
      l1_fill = darken(fill, 0.93)
      l2_fill = darken(fill, 0.86)
      ...
      l1_groups.keys.sort.each do |l1|
        ...
        sg.subgraph("cluster_#{l1_cid}",
                    label: l1, style: "rounded,filled", fillcolor: l1_fill,
                    fontname: "Helvetica", fontsize: 11) do |l1g|
          ...
          l1_groups[l1].keys.compact.sort.each do |l2|
            l2_cid = "#{cluster_id}_#{l2.gsub("/", "_")}".gsub(/[^a-zA-Z0-9]/, "_")
            l2_label = l2.include?("/") ? l2.split("/")[1] : l2
            l1g.subgraph("cluster_#{l2_cid}",
                         label: l2_label, style: "rounded,filled", fillcolor: l2_fill,
                         fontname: "Helvetica", fontsize: 10) do |l2g|
              l1_groups[l1][l2].sort.each do |pkg|
                parts = rel_parts(pkg, mod_name)
  ...
  def emit_clusters_single_module(g, mod_name, pkgs_in_mod, prefix)
    l1_groups, ungrouped = group_by_l1_l2(pkgs_in_mod, mod_name)

    l1_colors = l1_groups.keys.sort.each_with_index.each_with_object({}) do |(l1, idx), h|
      h[l1] = PALETTE[idx % PALETTE.length]
    end

    ungrouped.each { |pkg| g.node(node_id(pkg, prefix), label: short_label(pkg, mod_name)) }
    ...
    l1_groups.keys.sort.each do |l1|
      ...
      g.subgraph("cluster_#{l1_cid}",
                 label: l1, style: "rounded,filled", fillcolor: fill,
                 fontname: "Helvetica Bold", fontsize: 12) do |l1g|
        (l1_groups[l1][nil] || []).sort.each do |pkg|
          l1g.node(node_id(pkg, prefix), label: short_label(pkg, mod_name))
        end

        l1_groups[l1].keys.compact.sort.each do |l2|
          l2_cid = l2.gsub("/", "_").gsub(/[^a-zA-Z0-9]/, "_")
          l2_label = l2.include?("/") ? l2.split("/")[1] : l2
          l1g.subgraph("cluster_#{l2_cid}",
                       label: l2_label, style: "rounded,filled", fillcolor: l2_fill,
                       fontname: "Helvetica", fontsize: 10) do |l2g|
  ...
  def emit_dot(pkgs, modules, module_colors, prefix, ranksep: 0.6, nodesep: 0.15)
    ...
    single_module = modules.length == 1
    ...
    Archsight::Graphvis.new("packages",
                            ...
                            ranksep: ranksep, nodesep: nodesep,
                            fontname: "Helvetica", fontsize: 10).draw_dot do |g|
      g.defaults(:node, fontname: "Helvetica", fontsize: 8, shape: :box,
                        style: "rounded,filled", fillcolor: :white, height: 0.2, width: 0.4)
      g.defaults(:edge, fontname: "Helvetica", fontsize: 8)
      ...
      if single_module
        ...
        l1_colors = emit_clusters_single_module(g, mod_name, pkgs_in_mod, prefix)
        ...
        l1_rep = {}
        depths.each_key do |l1|
          pkgs_in_l1 = pkgs_in_mod.select { |p| rel_parts(p, mod_name)[0, 1] == [l1] }.sort
          l1_rep[l1] = node_id(pkgs_in_l1.first, prefix) if pkgs_in_l1.any?
        end

        l1_imports.each do |src_l1, dst_l1s|
          src_rep = l1_rep[src_l1]
          dst_l1s.each do |dst_l1|
            dst_rep = l1_rep[dst_l1]
            g.edge(src_rep, dst_rep, style: :invis, constraint: true) if src_rep && dst_rep
          end
        end
      else
        modules.each do |rel_dir, mod_name|
          ...
          emit_module_cluster(g, mod_name, module_colors[rel_dir], pkgs_in_mod, prefix)
          color = module_colors[rel_dir][:edge]
          pkgs_in_mod.each { |pkg| pkg_edge_color[pkg] = color }
        end

        _mod_depths, mod_imports = module_topo_depths(pkgs, modules)
        mod_rep = {}
        modules.each do |rel_dir, _|
          pkgs_in_mod = (by_dir[rel_dir] || []).sort
          mod_rep[rel_dir] = node_id(pkgs_in_mod.first, prefix) if pkgs_in_mod.any?
        end

        mod_imports.each do |src_dir, dst_dirs|
          src_rep = mod_rep[src_dir]
          dst_dirs.each do |dst_dir|
            dst_rep = mod_rep[dst_dir]
            g.edge(src_rep, dst_rep, style: :invis, constraint: true) if src_rep && dst_rep
          end
        end
      end
      ...
      hierarchy_edges.sort.each do |parent, child|
        g.edge(node_id(parent, prefix), node_id(child, prefix),
      ...
      pkgs.keys.sort.each do |pkg|
        ...
        pkgs[pkg].sort.each do |dep|
          next if dep == pkg
          g.edge(node_id(pkg, prefix), node_id(dep, prefix), color: edge_color, style: :solid)
        end
      end
    end
  end
